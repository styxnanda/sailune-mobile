...
adb logcat -c
flutter drive --driver test_driver/integration_test.dart --target integration_test/library_test.dart -d emulator-5554 2>&1 | tee "$log_path"
result=${PIPESTATUS[0]}
if [ "$result" -ne 0 ] && [ "${GITHUB_ACTIONS:-}" = true ]; then
  adb logcat -d -v brief '*:E' 'GoLog:V' 'DEBUG:F' | tail -150 >> "$log_path"
  python3 - "$log_path" <<'PY'
import pathlib, sys
